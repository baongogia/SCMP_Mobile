...
import { getCourses } from '@/api/member/courses';
...
export function CourseInfoPopup() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      const response = await getCourses(JSON.parse(tenant).value, token);
  ...
  const renderCourseItem = ({ item, index }: { item: any; index: number }) => {
    ...
    return (
      ...
      >
        <View style={[
          styles.cardContent,
          isExpanded && styles.expandedCardContent
        ]}>
    ...
    if (courses.length === 0) {
      return (
        <View style={styles.emptyContainer}>
          ...
          <ThemedText style={styles.emptyText}>Không có khóa học nào</ThemedText>
