...
import { getSchedule } from '@/api/member/schedule';
...
export function SchedulePopup() {
  ...
  const fetchMonthSchedule = async (date: Date) => {
    try {
      ...
      const response = await getSchedule(JSON.parse(tenant).value, token, gte, lt);
      setMonthScheduleData(response || []);
    } catch (err) {
                ...
                selectedDateSchedule.map((item, idx) => (
                  <View key={item._id || idx} style={styles.scheduleItem}>
                    <Text style={styles.itemTitle}>
                      {item.classroom?.[0]?.name || item.classroom?.name || 'Lớp học'}
                    </Text>
