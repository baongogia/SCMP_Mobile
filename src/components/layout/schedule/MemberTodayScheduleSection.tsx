...
interface MemberTodayScheduleSectionProps {
  onPress?: () => void;
}

export const MemberTodayScheduleSection: React.FC<
  MemberTodayScheduleSectionProps
> = ({ onPress }) => {
  ...
  const fetchTodaySchedules = useCallback(async () => {
    try {
      ...
      let res = await getAllMemberSchedules(todayStr, todayStr);
      ...
      if (todaySchedules.length === 0) {
        ...
        res = await getAllMemberSchedules(todayStr, nextWeekStr);
        items = res?.data?.data || [];
