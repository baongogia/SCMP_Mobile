...
const transformTransaction = (
  ...
  try {
    ...
    const status =
      apiTransaction.status && apiTransaction.status.length > 0
        ? apiTransaction.status[0]
        : "pending";
    ...
    return {
      ...
      date: new Date(
        new Date(
          apiTransaction.created_at || new Date().toISOString()
        ).getTime() -
          7 * 60 * 60 * 1000
      ).toISOString(),
...
export function PaymentHistoryPopup() {
  ...
  const loadTransactions = async (page: number = 1, reset: boolean = false) => {
    try {
      ...
      const response = await courseService.getPaymentHistory(tenant.value);
      ...
      if (Array.isArray(response)) {
        ...
        if (
          ...
          Array.isArray(response.data.data[0][0])
        ) {
      ...
      const transformedTransactions = apiData
        .map((transaction: ApiTransaction, index: number) => {
          try {
                  ...
                  <View style={styles.detailCard}>
                    ...
                    <View style={styles.detailItem}>
                      ...
                      <View style={styles.statusRow}>
                        <Ionicons
                          name={
                            getStatusIcon(selectedTransaction.status) as any
                          }
                          size={16}
