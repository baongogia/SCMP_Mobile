...
import { WeatherData, WeatherState, WeatherLocation } from "@/src/types/weather";
import { weatherService } from "@/src/services/weather/weatherService";
import { WEATHER_CONFIG } from "@/src/constants/config";
import { useLocation } from "./useLocation";

export const useWeather = (location?: WeatherLocation, useRealLocation: boolean = true) => {
  ...
  const fetchWeather = useCallback(async (weatherLocation?: WeatherLocation) => {
    ...
    if (!targetLocation) {
      setWeatherState(prev => ({
        ...prev,
        error: useRealLocation ? "Đang lấy vị trí..." : "Location not provided",
  ...
  }, [location, useRealLocation, realLocation, getWeatherLocation, locationLoading]);
  ...
  }, [realLocation?.latitude, realLocation?.longitude, location?.latitude, location?.longitude, fetchWeather, useRealLocation]);
...
export const useWeatherByCity = (cityName?: string) => {
  ...
  const fetchWeatherByCity = useCallback(async (city?: string) => {
    ...
    try {
      const weatherData = await weatherService.getCurrentWeatherByCity(targetCity);
      setWeatherState({
        data: weatherData,
        loading: false,
        error: null,
