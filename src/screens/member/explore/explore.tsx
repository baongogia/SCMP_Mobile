...
export default function Explore() {
  ...
  const [courseDetail, setCourseDetail] = useState<ApiCourseDetail | null>(
    null
  );
  ...
  const fetchCourses = async (
    ...
    try {
      ...
      const response = await courseService.getPublicCourses(queryParams);

      const apiCourses = response.data || [];
      const transformedCourses = apiCourses.map(
        (course: ApiCourse, index: number) => transformCourse(course, index)
      );
  ...
  const handleEnrollCourse = async () => {
    ...
    try {
      ...
      // Call order API
      const response = await courseService.orderCourse(
        tenant,
        courseDetail._id
      );
      ...
      <View style={styles.courseInfo}>
        ...
        <View style={styles.courseDetails}>
          <Text style={styles.sessionInfo}>
            {item.sessionNumber} buổi • {item.sessionDuration}
          </Text>
        </View>
        ...
        <Text style={styles.price}>
          {item.price.toLocaleString("vi-VN")}k VNĐ
        </Text>
      </View>
    </TouchableOpacity>
  );
  ...
  const getInstructorImage = () => {
    return (
      courseDetail?.created_by?.featured_image?.[0]?.path ||
    ...
    <View style={styles.container}>
      ...
      <View style={styles.resultsInfo}>
        <Text style={styles.resultsText}>
          {loading
            ? "Đang tải khóa học..."
            : `Hiển thị ${courses.length} trong ${
                metaData.count
              } khóa học (Trang ${currentPage}/${Math.max(totalPages, 1)})`}
        </Text>
      </View>
              ...
              <View style={styles.paginationContainer}>
                ...
                >
                  <Ionicons
                    ...
                    size={24}
                    color={
                      currentPage >= totalPages || loading ? "#ccc" : "#333"
                    }
                  />
                </TouchableOpacity>
              </View>
              ...
              {detailLoading ? (
                    ...
                    <View style={styles.modalStatsContainer}>
                      <View style={styles.modalStatItem}>
                        ...
                        <Text style={styles.modalStatText}>
                          {courseDetail.session_number} buổi
                        </Text>
                      </View>
                      <View style={styles.modalStatItem}>
                        <Ionicons
                        ...
                        />
                        <Text style={styles.modalStatText}>
                          {courseDetail.session_number_duration}
                        </Text>
                      </View>
                      <View style={styles.modalStatItem}>
                    ...
                    <View style={styles.modalInstructorContainer}>
                      ...
                      <View style={styles.modalInstructorInfo}>
                        ...
                        <View style={styles.modalInstructorDetails}>
                          <Text style={styles.modalInstructorName}>
                            {courseDetail.created_by.username}
                          </Text>
                          {courseDetail.created_by.email && (
                            <Text style={styles.modalInstructorEmail}>
                              {courseDetail.created_by.email}
                            </Text>
                          )}
                        </View>
                    ...
                    {courseDetail.lessons &&
                      courseDetail.lessons.length > 0 && (
                        <View style={styles.modalLessonsContainer}>
                          <Text style={styles.modalSectionTitle}>
                            Nội dung khóa học ({courseDetail.lessons.length} bài
                            học)
                          </Text>
                          {courseDetail.lessons.map((lesson, index) => (
                            ...
                            >
                              <View style={styles.modalLessonNumber}>
                                <Text style={styles.modalLessonNumberText}>
                                  {index + 1}
                                </Text>
                              </View>
                    ...
                    <View style={styles.modalMetaContainer}>
                      {courseDetail.created_at && (
                        <Text style={styles.modalMetaText}>
                          Ngày tạo: {formatDate(courseDetail.created_at)}
                        </Text>
                      )}
                      {courseDetail.updated_at && (
                        <Text style={styles.modalMetaText}>
                          Cập nhật: {formatDate(courseDetail.updated_at)}
                        </Text>
                      )}
                    </View>
                  </View>
                ...
                <View style={styles.modalErrorContainer}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      selectedCourse && fetchCourseDetail(selectedCourse.id)
                    }
                  >
              ...
              <View style={styles.modalFooter}>
                <TouchableOpacity
                  style={[
                    ...
                    enrolling && styles.modalEnrollButtonDisabled,
                  ]}
                ...
                >
                  {enrolling ? (
                    ...
                    <>
                      ...
                      <Text style={styles.modalEnrollButtonPrice}>
                        {formatPrice(courseDetail.price)}
                      </Text>
                    </>
                  )}
